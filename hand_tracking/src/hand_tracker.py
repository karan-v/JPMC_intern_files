...
class HandTracker():
    ...
    def detect_hand(self, img_norm):
        ...
        box = []
        if(len(box_ids)==1):
            box.append(box_ids[0])
        else:
            box.append(box_ids[0])
            box.append(box_ids[1])

        # bounding box offsets, width and height
        sources,keypointss,deb = [],[],[]
        for box_id in box:
            ...
            deb.append(debug_info)

        return sources, keypointss, deb
    ...
    def __call__(self, img):
        ...
        for i in range(len(source)):
            ...
            kp_orig -= pad[::-1]
            box_orig -= pad[::-1]
            kp_origs.append(kp_orig)
            box_origs.append(box_orig)

        return kp_origs, box_origs
